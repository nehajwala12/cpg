...
eight exhibits across Brand Health, Merchandising, Trade & Retail Media, and
Supply Chain, and the CreditSwan brand system (V1): Cormorant Garamond
display, Manrope body, IBM Plex Mono data; ink tab band with a single Gilt
Gold active state; Heron Teal labels. Fonts are self-hosted in `fonts/` —
no external requests — and the CSP in `vercel.json` is locked to 'self'
accordingly. Content is unchanged from Rev 2 — no new findings raised;
